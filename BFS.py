...
def exploring_nodes(node):
    ...
    checked_node = []
    checked_node.append(queue_node[0].data.tolist())
    node_count = int(0)
    
    while queue_node:
        current_root = queue_node.pop(0)
        if current_root.data.tolist() == goal_state.tolist():
            print("Goal reached")
            return current_root
        
        for move in actions:
            temp_data = move_title(move, current_root.data)
            if temp_data is not None:
                node_count += 1
                child_node = Node(node_count, temp_data, current_root, move, 0)
