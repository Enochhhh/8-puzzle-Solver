...
def exploring_nodes(node):
    ...
    while stack_node:
        while stack_node:
            ...
            if current.data.tolist() == goal_state.tolist():
                return current
            checked_node.append(current)
            if current.cost % 10 == 0 and current.cost != 0:
                queue_node.append(current)
                continue            
            for move in actions:
                temp_data = move_title(move, current.data)
                if temp_data is not None:
                    node_count += 1
                    children = Node(node_count, temp_data, current, move, current.cost + 1)
                    if children not in stack_node and children not in checked_node:
                        stack_node.append(children)
                    else:
                        node_count -= 1        
